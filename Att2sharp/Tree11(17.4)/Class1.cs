...
namespace SpaceTree
{
    ...
    class MyTree
    {
        ...
        int Lp = 0;
        public string[] Neg = new string[0];
        int Ln = 0;
        public int Depth = 1;
        ...
        public void FillNode(Node node)
        {
            if (node.depth < Depth)
            {
                ...
                node.A.routeweight = node.routeweight + 1;
                ...
                node.B.routeweight = node.routeweight -1;
            ...
            {
                if (node.routeweight < 0) { node.positive = false; }
                if (node.routeweight > 0) { node.negative = false; }
                //Console.WriteLine(node.route);
                if (node.positive == true)
                {
                    Array.Resize<string>(ref Pos, ++Lp);
                    Pos[Lp - 1] = node.route;
                }
                if (node.negative == true)
                {
                    Array.Resize<string>(ref Neg, ++Ln);
                    Neg[Ln - 1] = node.route;
                }
            }
        }
